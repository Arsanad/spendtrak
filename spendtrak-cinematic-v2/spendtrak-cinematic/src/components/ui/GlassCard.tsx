...
export type CardVariant = 'default' | 'elevated' | 'outlined' | 'filled' | 'glow';
...
export const GlassCard: React.FC<GlassCardProps> = ({
  children, variant = 'default', size = 'default', style, onPress, onLongPress, disabled = false, animated = true, noPadding = false,
}) => {
  ...
  const cardContent = (
    <>
      <LinearGradient colors={variantStyles.gradientColors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
      ...
      <View style={[styles.contentWrapper, !noPadding && paddingStyle]}>{children}</View>
    </>
  );
  ...
  if (onPress || onLongPress) {
    return (
      <AnimatedPressable onPress={disabled ? undefined : onPress} onLongPress={disabled ? undefined : onLongPress} onPressIn={handlePressIn} onPressOut={handlePressOut} style={[baseStyle, animated && animatedStyle]}>
        {cardContent}
      </AnimatedPressable>
    );
  }
...
// Quick Action Card
export const QuickActionCard: React.FC<{ children: React.ReactNode; onPress?: () => void; active?: boolean; style?: ViewStyle }> = ({ children, onPress, active = false, style }) => (
  <GlassCard variant={active ? 'glow' : 'default'} size="compact" onPress={onPress} style={[quickStyles.card, style]}>{children}</GlassCard>
);

// Stats Card
export const StatsCard: React.FC<{ children: React.ReactNode; onPress?: () => void; style?: ViewStyle }> = ({ children, onPress, style }) => (
  <GlassCard variant="elevated" onPress={onPress} style={style}>{children}</GlassCard>
);

// List Item Card
export const ListItemCard: React.FC<{ children: React.ReactNode; onPress?: () => void; style?: ViewStyle }> = ({ children, onPress, style }) => (
